...
export const useCeoUsersStore = defineStore('ceo-users', {
  state: (): UsersState => ({
    users: [],
    connectedUsers: [],
    loading: false,
  ...
  actions: {
    async fetchAll(params: object = {}) {
      ...
      try {
        const response = await ceoServices.getAllUsers(params)
    ...
    async fetchConnected(minutes: number = 5) {
      ...
      try {
        const response = await ceoServices.getConnectedUsers(minutes)
    ...
    async fetchById(id: number) {
      ...
      try {
        const response = await ceoServices.getUserById(id)
        return { success: true, data: response.data }
      } catch (error: any) {
    ...
    async updateUser(id: number, data: Partial<User>) {
      ...
      try {
        const response = await ceoServices.updateUser(id, data)
    ...
    async deleteUser(id: number) {
      ...
      try {
        await ceoServices.deleteUser(id)
        // Remover del state
